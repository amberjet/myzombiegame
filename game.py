...
class Brains(pygame.sprite.Sprite):  # тут создаются мозги
    def __init__(self, min_speed, max_speed, variant):
        ...
        # скорость у экземпляров мозгов постоянная
        self.speed_y = random.randrange(min_speed, self.max_speed)
        self.speed_x = random.randrange(-3, 3)

    def update(self):
        # двигаем мозги. если добрались до стены - отталкиваются
...
class Zombie(pygame.sprite.Sprite):  # тут создается зомби-игрок
    ...
    # тут обновляем зомби с учетом движения и меняем его анимацию
    def update(self):
        # меняем анимацию
        if self.vid <= 5:
...
def draw_text(surf, text, size, x, y):  # отрисовываем тексты
...
def check_brain():  # не очень хорошо, если мозг появится прямо под зомби
    m = new_brain()
    if m.rect.right > player.rect.left and \
            m.rect.left < player.rect.right and \
            m.rect.bottom > player.rect.top and \
            m.rect.top < player.rect.bottom:
        m = new_brain()
    return m
# да, можно было это сделать через collide, но они сравнивает объект с группой
# а мне нужно проверить, нет окажется ли один новый мозг под зомби
...
pygame.display.set_caption("Зомби в зимнем лесу собирает мозги!")
...
heart = pygame.image.load('files/heart.png').convert_alpha()
...
while running:
    if start_screen:
        SCORE = 0  # счет
        LIFES = 2  # если успею, перенести в класс игрока
    ...
    draw_text(screen, 'Осталось жизней:' + str(LIFES + 1), 18, 750, 20)
    pygame.display.flip()
pygame.quit()
